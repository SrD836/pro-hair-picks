...
export function CookieBanner() {
  ...
  const acceptAll = () => saveConsent({ necessary: true, analytics: true, advertising: true, timestamp: Date.now() });
  const rejectAll = () => saveConsent({ necessary: true, analytics: false, advertising: false, timestamp: Date.now() });
  const saveConfig = () => saveConsent({ necessary: true, analytics, advertising, timestamp: Date.now() });
        ...
        {!showConfig ? (
          <>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-4">
              <Button onClick={acceptAll} size="sm" className="bg-primary text-primary-foreground">{t("cookies.acceptAll")}</Button>
              <Button onClick={rejectAll} size="sm" variant="outline">{t("cookies.reject")}</Button>
              <Button onClick={() => setShowConfig(true)} size="sm" variant="ghost" className="gap-1">
                <Settings className="w-3 h-3" /> {t("cookies.configure")}
              </Button>
            </div>
          </>
        ) : (
          <>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-display font-bold text-foreground text-sm">{t("cookies.configTitle")}</h3>
            ...
            <div className="space-y-3">
              <label className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                <div>
                  <p className="text-sm font-medium text-foreground">{t("cookies.technical")}</p>
                  <p className="text-xs text-muted-foreground">{t("cookies.technicalDesc")}</p>
                </div>
              ...
              <label className="flex items-center justify-between p-3 bg-muted/50 rounded-lg cursor-pointer">
                <div>
                  <p className="text-sm font-medium text-foreground">{t("cookies.advertising")}</p>
            ...
            <div className="flex gap-2 mt-4">
              <Button onClick={saveConfig} size="sm" className="bg-primary text-primary-foreground">{t("cookies.savePreferences")}</Button>
              <Button onClick={acceptAll} size="sm" variant="outline">{t("cookies.acceptAll")}</Button>
            </div>
          </>
        )}
      </div>
