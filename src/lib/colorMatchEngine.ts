...
// Fantasy / dyed color codes (negative numbers to distinguish from natural levels)
...
export interface UserProfile {
  ...
  currentLevel: number; // 1-10 for natural, 0 for fantasy
...
const COLOR_DB: ColorEntry[] = [
  ...
  // NEW: Expanded redhead & copper range for Autumn
  ...
  // NEW: Light warm coppers/golds for Spring
...
export function getRecommendation(profile: UserProfile): ColorRecommendation {
  ...
  const effectiveCurrentLevel = isFantasy ? 0 : profile.currentLevel;
  ...
  const scored = COLOR_DB.map((c) => {
    ...
    // Boost reds/coppers for warm Autumn/Spring profiles seeking warmth
...
function buildVerdictEs(season: Season, profile: UserProfile, color: ColorEntry, isFantasy: boolean, requiresDecolor: boolean): string {
  ...
  if (isFantasy) {
    base += ` ⚠️ Tu cabello actualmente tiene un color fantasía/teñido. Para lograr este resultado será imprescindible un proceso de decoloración y/o decapado profesional previo. No intentes este cambio en casa: acude a un colorista profesional que evalúe el estado de tu fibra capilar.`;
  } else if (requiresDecolor) {
    base += ` ⚠️ Tu cabello actual es más oscuro que el tono recomendado y el salto de nivel es considerable. Necesitarás un proceso de decoloración previo realizado por un profesional para proteger la integridad de tu cabello.`;
  }
...
function buildVerdictEn(season: Season, profile: UserProfile, color: ColorEntry, isFantasy: boolean, requiresDecolor: boolean): string {
  ...
  if (isFantasy) {
    base += ` ⚠️ Your hair currently has a fantasy/dyed color. Achieving this result will require a professional bleaching and/or color removal process. Do not attempt this at home — visit a professional colorist who can assess your hair fiber condition.`;
  } else if (requiresDecolor) {
    base += ` ⚠️ Your current hair is darker than the recommended shade and the level jump is significant. You'll need a professional bleaching process first to protect your hair's integrity.`;
  }
